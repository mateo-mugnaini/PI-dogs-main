...
export function getBreeds(type = "All") {
  ...
  return async function (dispatch) {
    const response = await axios.get(`http://localhost:3001/dogs?type=${type}`);
    dispatch({
      type: GET_BREEDS,
...
export function getBreed(name, type) {
  return async function (dispatch) {
    const response = await axios.get(
      `http://localhost:3001/dogs?name=${name}&type=${type}`
    );
    dispatch({
      type: GET_BREED,
      payload: response.data,
...
export function filterByTemperaments(tempName, type) {
  return async function (dispatch) {
    const response = await axios.get(
      `http://localhost:3001/dogs?tempName=${tempName}&type=${type}`
    );
    dispatch({
      type: GET_BREEDS,
      payload: response.data,
